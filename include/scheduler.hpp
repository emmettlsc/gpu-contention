...
enum class KernelType {
    MEMORY_BOUND,    // canny edge detection, gaussian blur, etc.
    COMPUTE_BOUND,   // harris corner detection, matmul, conv2d
    ATOMIC_BOUND,    // histogram
    MIXED            // mixed workload characteristics
};

// GPU architecture classifications for threshold tuning
enum class GPUArchitecture {
    TURING,      // T4 (2018)
    AMPERE,      // A10G, A100 (2020-2021)
    ADA,         // L4 (2022)
    UNKNOWN
};
...
struct KernelSpec {
    ...
    // Extended fields for better scheduling decisions
    float arithmetic_intensity;      // flops per byte
    float estimated_sm_usage_pct;    // estimated SM utilization %
    ...
    // Default constructor
    KernelSpec()
        : type(KernelType::MIXED), name("unknown"), estimated_blocks(0),
          estimated_bandwidth_gb_s(0.0f), arithmetic_intensity(0.0f),
          estimated_sm_usage_pct(50.0f), input_size_bytes(0),
          output_size_bytes(0), temp_size_bytes(0) {}
    ...
    KernelSpec(KernelType t, const std::string& n, int blocks, float bandwidth)
        : type(t), name(n), estimated_blocks(blocks), estimated_bandwidth_gb_s(bandwidth),
          arithmetic_intensity(0.0f), estimated_sm_usage_pct(50.0f),
          input_size_bytes(0), output_size_bytes(0), temp_size_bytes(0) {}

    // Extended constructor
    KernelSpec(KernelType t, const std::string& n, int blocks, float bandwidth,
               float arith_int, float sm_usage)
        : type(t), name(n), estimated_blocks(blocks), estimated_bandwidth_gb_s(bandwidth),
          arithmetic_intensity(arith_int), estimated_sm_usage_pct(sm_usage),
          input_size_bytes(0), output_size_bytes(0), temp_size_bytes(0) {}
};
...
struct SchedulerConfig {
    std::string gpu_name;
    std::string architecture;  // human-readable name
    GPUArchitecture arch_enum; // enum for programmatic use
...
public:
    ...
    static GPUArchitecture detect_gpu_architecture(int major_cc, int minor_cc, const std::string& gpu_name);
    static std::string architecture_to_string(GPUArchitecture arch);
