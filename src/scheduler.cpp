...
GPUArchitecture Scheduler::detect_gpu_architecture(int major_cc, int minor_cc, const std::string& gpu_name) {
    // Detect by GPU name first
    if (gpu_name.find("T4") != std::string::npos) {
        return GPUArchitecture::TURING;
    }
    if (gpu_name.find("L4") != std::string::npos || gpu_name.find("L40S") != std::string::npos) {
        return GPUArchitecture::ADA;
    }
    if (gpu_name.find("3060") != std::string::npos || gpu_name.find("RTX 3060") != std::string::npos) {
    ...
    // Ampere: 8.6 (RTX 3060)
    // Ada: 8.9 (L4, L40S)
    if (major_cc == 7 && minor_cc == 5) {
        return GPUArchitecture::TURING;
    }
    if (major_cc == 8 && minor_cc == 6) {
        return GPUArchitecture::AMPERE;
    }
    if (major_cc == 8 && minor_cc == 9) {
        return GPUArchitecture::ADA;
    }
...
std::string Scheduler::architecture_to_string(GPUArchitecture arch) {
    switch (arch) {
...
SchedulerConfig Scheduler::create_config_for_gpu(const GPUHardwareInfo& gpu_info) {
    ...
    // Detect architecture
    config.arch_enum = detect_gpu_architecture(
        gpu_info.major_compute_capability,
        gpu_info.minor_compute_capability,
        gpu_info.name
    );
    config.architecture = architecture_to_string(config.arch_enum);

    // Architecture-specific threshold tuning for: T4, L4, RTX 3060, L40S
    switch (config.arch_enum) {
        case GPUArchitecture::TURING:  // T4: 320 GB/s, 40 SMs
        ...
        case GPUArchitecture::AMPERE:  // RTX 3060: 360 GB/s, 28 SMs
            config.memory_bandwidth_threshold_gb_s = 240.0f;
            config.sm_utilization_threshold_percent = 68.0f;
            config.memory_utilization_threshold_percent = 80.0f;
            config.min_concurrent_image_pixels = 1382400;  // 1080p
            config.max_blocks_per_sm = 7;
            config.thermal_threshold_c = 85.0f;
            config.memory_pressure_weight = 0.48f;
            config.sm_occupancy_weight = 0.32f;
            config.thermal_weight = 0.15f;
            config.workload_complementarity_weight = 0.05f;
            break;

        case GPUArchitecture::ADA:  // L4: 300 GB/s, 58 SMs | L40S: 864 GB/s, 142 SMs
            if (gpu_info.memory_bandwidth_gb_s > 600.0f) {  // L40S
                config.memory_bandwidth_threshold_gb_s = 550.0f;
                config.sm_utilization_threshold_percent = 65.0f;
            } else {  // L4
                config.memory_bandwidth_threshold_gb_s = 200.0f;
                config.sm_utilization_threshold_percent = 70.0f;
            }
        ...
        default:  // Unknown GPU - conservative defaults
            config.memory_bandwidth_threshold_gb_s = gpu_info.memory_bandwidth_gb_s * 0.6f;
            config.sm_utilization_threshold_percent = 70.0f;
            config.memory_utilization_threshold_percent = 80.0f;
